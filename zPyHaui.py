...
ctdk = input("Nhập vào mã ngành(Mặc đinh là CNTT): ")
properties = ['IndependentClassID','ModulesName', 'ClassCode', 'CountS', 'MaxStudent', 'ListDate']

# URL và dữ liệu yêu cầu
addurl = f'https://sv.haui.edu.vn/ajax/register/action.htm?cmd=addclass&v={kverify if kverify else "DAC31F70B5AA0E1D5B8CDEFD337C1D72"}'
listurl = f'https://sv.haui.edu.vn/ajax/register/action.htm?cmd=classbymodulesid&v={kverify if kverify else "DAC31F70B5AA0E1D5B8CDEFD337C1D72"}'
...
def addClass(class_id):
    ...
    # Gửi yêu cầu POST
    response = requests.post(addurl, data=payload, headers=headers)
...
def classCamp(idSubj):
    class_id = input("Nhập vào id lớp cần camp: ")
    data = {'fid': int(idSubj) if idSubj else 6819}
    print(f"Đang camp môn......")
    while True:
        try:
            listresponse = requests.post(listurl, data=data, headers=headers)
            json_data = listresponse.json()
            if json_data.get('data') and len(json_data['data']) > 0:
                result = next((obj for obj in json_data['data'] if obj["IndependentClassID"] == int(class_id)), None)
                if result['CountS'] < result['MaxStudent']: 
                    addClass(class_id= class_id)
                    print('Camp Thành công')
                    break
                else:
                    print(f"{result['CountS']}/{result['MaxStudent']}", end="\t")
            time.sleep(3)
        except ValueError:
...
def classList(idSubj):
    try:
        # Gửi yêu cầu POST
        data = {'fid': int(idSubj) if idSubj else 6819}
        response = requests.post(listurl, data=data, headers=headers)
